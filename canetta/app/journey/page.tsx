        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto" }}>
          ...
          {st.registerFlow === "nutricao" && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "20px 24px 24px", overflowY: "auto" }}>
              ...
              {st.nutritionStep === 5 && <div><div style={{ fontSize: 20, fontWeight: 900, color: "#16302B", lineHeight: 1.15, marginBottom: 16 }}>Você tem uma meta de proteína para acompanhar?</div><div style={{ fontSize: 12.5, color: "#4B5F59", lineHeight: 1.4, marginBottom: 16 }}>Informe apenas uma meta já combinada com seu profissional ou que você já decidiu acompanhar. O Canetta não calcula uma meta clínica para você.</div><ChipRow options={["Tenho uma meta", "Não tenho", "Ainda não sei"]} current={st.draft.metaProteinaFonte} onPick={(v) => setDraft({ metaProteinaFonte: v })} equal />{st.draft.metaProteinaFonte === "Tenho uma meta" && <div style={{ marginTop: 16 }}><div style={fieldLabel}>META DE PROTEÍNA (GRAMAS/DIA)</div><input className="j-in" type="number" min={0} max={500} value={st.draft.metaProteinaGramas ?? ""} onChange={(e) => setDraft({ metaProteinaGramas: Number(e.target.value) })} style={inputSt} placeholder="Ex: 90" /></div>}<div style={{ ...fieldLabel, marginTop: 18 }}>ÁGUA NO DIA (COPOS)</div><input className="j-in" type="number" min={0} max={50} value={st.draft.agua ?? ""} onChange={(e) => setDraft({ agua: Number(e.target.value) })} style={inputSt} placeholder="Ex: 6" /><div style={{ ...fieldLabel, marginTop: 18 }}>REFEIÇÃO (OPCIONAL)</div><input className="j-in" value={st.draft.refeicao || ""} onChange={(e) => setDraft({ refeicao: e.target.value })} style={inputSt} placeholder="Ex: almoço" /><textarea className="j-in" value={st.draft.nota || ""} onChange={(e) => setDraft({ nota: e.target.value })} placeholder="Nota opcional" style={{ ...textareaSt, marginTop: 14 }} /></div>}
