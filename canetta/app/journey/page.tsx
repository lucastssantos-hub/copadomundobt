                  ...
                  <>
                    ...
                    {(!aiTraining || anamneseOpen) ? (
                      <div style={{ ...cardWhite, display: "flex", flexDirection: "column", gap: 14 }}>
                        ...
                        {anamneseStep === 0 && <div><div style={{ fontSize: 18, fontWeight: 800, color: "#16302B", marginBottom: 10 }}>Qual é sua experiência?</div><ChipRow options={["Nunca treinei", "Já treinei, parei", "Treino regularmente"]} current={anamneseLevel} onPick={(v) => { setAnamneseLevel(v); setAnamneseStep(1); }} wrap /></div>}
                        {anamneseStep === 1 && <div><div style={{ fontSize: 18, fontWeight: 800, color: "#16302B", marginBottom: 10 }}>Onde você vai treinar?</div><ChipRow options={["Casa, sem equipamento", "Casa, com equipamento", "Academia"]} current={anamneseLocation} onPick={(v) => { setAnamneseLocation(v); setAnamneseStep(2); }} wrap /></div>}
                        {anamneseStep === 2 && <div><div style={{ fontSize: 18, fontWeight: 800, color: "#16302B", marginBottom: 10 }}>Quantos dias por semana?</div><ChipRow options={["2", "3", "4"]} current={anamneseDays} onPick={(v) => { setAnamneseDays(v); setAnamneseStep(3); }} equal /></div>}
                        {anamneseStep === 3 && <div><div style={{ fontSize: 18, fontWeight: 800, color: "#16302B", marginBottom: 10 }}>Quanto tempo por sessão?</div><ChipRow options={["30 min", "45 min", "60 min"]} current={anamneseMinutes} onPick={(v) => { setAnamneseMinutes(v); setAnamneseStep(4); }} equal /></div>}
