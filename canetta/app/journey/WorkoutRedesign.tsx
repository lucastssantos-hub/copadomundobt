      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {isYellow && (
          <div style={{ padding: "11px 13px", background: colors.warningBg, border: `1.5px solid ${colors.warningLine}`, borderRadius: 14 }}>
            ...
            <div style={{ color: colors.warning, fontSize: 12.5, lineHeight: 1.45 }}>Sessões de baixa demanda esta semana. Faça o check-in antes de iniciar.</div>
          </div>
        )}
        ...
        <section style={{ background: colors.pine, borderRadius: 16, padding: "18px 18px 16px", color: colors.onDark }}>
          ...
          <button type="button" onClick={() => setExecution({ day: selectedDay, exercise: 0 })} style={{ width: "100%", border: "none", borderRadius: 14, background: colors.onDark, color: colors.pine, padding: 14, fontSize: 14.5, fontWeight: 800, cursor: "pointer" }}>
            {completedSets ? "Continuar sessão" : "Iniciar treino"}
            <span style={{ display: "block", fontSize: 11, fontWeight: 700, color: colors.pine, opacity: 0.66, marginTop: 3 }}>~{sessionMinutes} min · {exercises.length} exercícios</span>
          </button>
        </section>
        ...
        <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 2 }}>
          {exercises.map((exercise, index) => {
            ...
            return (
              <button key={`${exercise.name}-${index}`} type="button" onClick={() => media ? onOpenMedia({ url: media, name: exerciseLabel(exercise) }) : setExecution({ day: selectedDay, exercise: index })} style={{ flex: "0 0 86px", border: "none", background: "transparent", padding: 0, textAlign: "left", cursor: "pointer" }}>
        ...
        <div style={{ background: colors.surface, border: `1.5px solid ${colors.line}`, borderRadius: 16, overflow: "hidden" }}>
          {exercises.map((exercise, index) => {
            ...
            return (
              <button key={`${exercise.name}-check-${index}`} type="button" onClick={() => setExecution({ day: selectedDay, exercise: index })} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "13px 15px", border: "none", borderBottom: index === exercises.length - 1 ? "none" : `1px solid ${colors.line}`, background: "transparent", textAlign: "left", cursor: "pointer" }}>
        ...
        <div role="dialog" aria-modal="true" aria-label="Modo de execução" style={{ position: "fixed", inset: 0, zIndex: 50, background: colors.bg, overflowY: "auto", padding: "18px 18px 28px" }}>
          <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100%", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button type="button" onClick={() => setExecution(null)} aria-label="Fechar modo de execução" style={{ marginLeft: 14, width: 36, height: 36, borderRadius: 12, border: `1.5px solid ${colors.line}`, background: colors.surface, color: colors.ink, fontSize: 20, cursor: "pointer" }}>×</button>
            </div>
            ...
            <div style={{ display: "flex", gap: 9, marginTop: 24 }}>
              <button type="button" disabled={execution.exercise === 0} onClick={() => setExecution((current) => current ? ({ ...current, exercise: Math.max(0, current.exercise - 1) }) : current)} style={{ flex: 1, padding: 13, borderRadius: 13, border: `1.5px solid ${colors.line}`, background: colors.surface, color: colors.soft, fontSize: 13, fontWeight: 800, cursor: execution.exercise === 0 ? "not-allowed" : "pointer", opacity: execution.exercise === 0 ? 0.45 : 1 }}>Anterior</button>
              <button type="button" onClick={() => execution.exercise < executionWorkout.exercises.length - 1 ? setExecution({ ...execution, exercise: execution.exercise + 1 }) : setExecution(null)} style={{ flex: 1, padding: 13, borderRadius: 13, border: "none", background: colors.pine, color: colors.onDark, fontSize: 13, fontWeight: 800, cursor: "pointer" }}>{execution.exercise < executionWorkout.exercises.length - 1 ? "Próximo" : "Concluir"}</button>
            </div>
          </div>
        </div>
      )}
